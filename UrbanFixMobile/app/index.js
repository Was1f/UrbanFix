...
const CommunityHome = () => {
  ...
  const [reportedMap, setReportedMap] = useState({}); // { [discussionId]: true }
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Mark only discussions that are currently pending reports on the admin side
      // We rely on the discussion.status === 'flagged' to indicate pending review
      const nextReported = {};
  ...
  const handleReport = async (discussionId, title) => {
    try {
      ...
      if (response.status === 201) {
        ...
        Alert.alert(
          'Already reported',
          'This discussion is already pending review. Do you want to revoke your report?',
          [
  ...
  const handleRevoke = (discussionId) => {
    Alert.alert(
      'Revoke report?',
      'This post is being reviewed by our admin. Do you want to revoke the report?',
      [
        ...
        {discussions.length > 0 ? (
          discussions.slice(0, 10).map((d, index) => (
            <View style={styles.discussionCard} key={d._id || index}>
              ...
              {d.image && (
                <Image source={{ uri: d.image }} style={styles.discussionImage} />
              )}
